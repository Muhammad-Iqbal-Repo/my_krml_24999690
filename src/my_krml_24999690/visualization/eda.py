...
def plot_distribution(df, col, hue=None, title=None, figsize=(8, 5)):
    ...
    ax = sns.histplot(data=df, x=col, hue=hue, kde=True, palette='Set2' if hue else None)
...
def plot_boxplot(df, cat_col, num_col, hue=None, title=None, figsize=(8, 5)):
    ...
    plt.figure(figsize=figsize)
    ax = sns.boxplot(data=df, x=cat_col, y=num_col, hue=hue, palette='Set2')
...
def plot_scatter(df, x_col, y_col, hue=None, trendline=False, title=None, figsize=(8, 5)):
    ...
    if trendline and not hue:
        ...
        ax = sns.regplot(data=df, x=x_col, y=y_col, scatter_kws={'alpha':0.6}, line_kws={'color':'red'})
    else:
        ax = sns.scatterplot(data=df, x=x_col, y=y_col, hue=hue, palette='Set2', alpha=0.7)
...
def plot_correlation_heatmap(df, columns=None, title=None, figsize=(10, 8)):
    ...
    ax = sns.heatmap(corr, mask=mask, annot=True, fmt=".2f", cmap='coolwarm', vmin=-1, vmax=1, square=True)
...
def plot_pairplot(df, columns=None, hue=None, title=None):
    ...
    g = sns.pairplot(data, hue=hue, palette='Set2', corner=True)
